...
class FLAME(nn.Module):
    ...
    def resize_params_for_batch(self, params, target_size):
        """resize the params to the target size by repeating or slicing the params"""
        current_size = params.shape[0]
        if current_size < target_size: # Case 1: we need to enlarge the params tensor
            repeats = [target_size // current_size + 1] + [1] * (params.dim() - 1)
            repeated_params = params.repeat(*repeats)
            new_params = torch.nn.Parameter(repeated_params[:target_size])
            return new_params
        elif current_size > target_size: # Case 2: we need to shrink the params tensor
            new_params = torch.nn.Parameter(params[:target_size])
            return new_params
        return params # Case 3: params tensor is already the correct size
    ...
    def forward(self, shape_params=None, expression_params=None, pose_params=None, neck_pose=None, eye_pose=None, transl=None):
        ...
        if shape_params.shape.__len__() == 3:

            # we have a sequence so we should colapse the first two dimensions
            new_batch_size = shape_params.shape[0] * shape_params.shape[1]
            self.batch_size = new_batch_size
            if self.expression_betas.shape[0] != new_batch_size:
                self.expression_betas = self.resize_params_for_batch(self.expression_betas, new_batch_size)
                self.shape_betas = self.resize_params_for_batch(self.shape_betas, new_batch_size)
                self.neck_pose = self.resize_params_for_batch(self.neck_pose, new_batch_size)
                self.eye_pose = self.resize_params_for_batch(self.eye_pose, new_batch_size)
                self.transl = self.resize_params_for_batch(self.transl, new_batch_size)
                
            shape_params = shape_params.view(-1, shape_params.shape[-1])
            expression_params = expression_params.contiguous().view(-1, expression_params.shape[-1])
            if pose_params.shape[2] == 3:
                # add a column of zeros to the pose params if we are only using jaw rotation
                pose_params = torch.cat([torch.zeros(pose_params.shape[0], pose_params.shape[1], 3, device=pose_params.device),pose_params], dim=2)
            pose_params = pose_params.view(-1, pose_params.shape[-1])
        # print("shape_params", shape_params.shape)
        # print("expression_params", expression_params.shape)
        # print("pose_params", pose_params.shape)
        # print("self.shape_betas shape", self.shape_betas.shape)
        # print("self.expression_betas shape", self.expression_betas.shape)
        ...
        # print("template_vertices", template_vertices.shape)
